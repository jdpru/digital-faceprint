...
heroCaption: A FreeSurfer reconstruction of one hemisphere's white-matter surface, lateral (left) and medial (right). Before any function is measured, each brain is rebuilt into anatomy like this.
...
<Figure src="/gallery/ontologies-of-cognitive-control/freesurfer-segmentation.jpg" caption="One scan after FreeSurfer, seen in FSLeyes. The cortical ribbon and subcortical structures have been segmented from the raw T1 scan, shown in color at left and as a grayscale label map at right. This anatomy is the coordinate frame every functional measurement is read against." />
